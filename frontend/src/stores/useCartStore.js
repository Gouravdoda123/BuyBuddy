...
export const useCartStore = create((set, get) => ({
	...
	getCartItems: async () => {
		try {
			...
			const data = res?.data;
			// ✅ Accept both shapes: [] or { cart: [] }
			const normalizedCart = Array.isArray(data) ? data : Array.isArray(data?.cart) ? data.cart : [];
	...
	removeFromCart: async (productId) => {
		try {
			...
			set((prevState) => ({ cart: prevState.cart.filter((item) => item?._id !== productId && item?.productId !== productId) }));
			get().calculateTotals();
		} catch (error) {
			toast.error(error?.response?.data?.message || "An error occurred");
		}
